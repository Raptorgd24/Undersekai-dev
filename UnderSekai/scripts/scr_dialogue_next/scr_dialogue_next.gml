...
function scr_dialogue_next() {
    ...
    if (!variable_global_exists("dialogue_personajes") || !is_array(global.dialogue_personajes)) {
        global.dialogue_personajes = [];
    }
    if (!variable_global_exists("dialogue_mensajes") || !is_array(global.dialogue_mensajes)) {
        global.dialogue_mensajes = [];
    }
    if (!variable_global_exists("dialogue_caras") || !is_array(global.dialogue_caras)) {
    ...
    if (global.dialogue_current >= array_length(global.dialogue_mensajes)) {
        ...
        show_debug_message("[Dialogue] Diálogo terminado, jugador liberado");
        return;
    }
    ...
    if (snd_id != -1) {
        ...
        global.dialogue_sound = -1;
    }

    // Pasar al siguiente mensaje
    global.dialogue_current += 1;
    
    show_debug_message("[Dialogue] Mostrando mensaje " + string(i) + " de " + string(array_length(global.dialogue_mensajes)) + ": " + string(global.dialogue_text));
}